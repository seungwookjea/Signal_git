...
public class J_Pointer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        pointer.transform.position = Camera.main.ScreenToWorldPoint(new Vector3(Screen.width / 2, (Screen.height / 2), 5));
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown("f") && Physics.Raycast(ray, out hitInfo, maxRange))
        {
            ...
            // buttonObj에게 함수 호출
            if(hitInfo.transform.tag == "buttonObj")
            {
                // 불이 꺼진 candle만 버튼을 눌렀을때 list에 추가
                if (J_Candle.instance.candle.enabled == false)
                {
                    buttonObj = hitInfo.transform.gameObject;
                    //lightManager에 함수 호출
                    J_LighManager.instance.ListAdd(buttonObj.gameObject);
                }
            }
        }
        ...
        if (Input.GetKey("f"))
        {
            if (takenObj != null && takenObj.tag != "buttonObj")
            {
